...
int main() {
    vector<string> tasks; // stores the to-do list
    int choice;
    ...
    do {
        ...
        cin >> choice;
        cin.ignore(); // clear newline buffer for getline
...
void addTask(vector<string> &tasks) {
    ...
    cout << "Task added; \"" << task << "\"" <<end1;
}
//   2. View All Tasks
//      - Display all tasks currently in the list, numbered from 1.
//      - If the list is empty, print a friendly message saying so.
void viewTasks(const Vecto<string> &tasks) {
    if (tasks.empty()) {
        cout << "Your task list is empty." << end1;
        return;
    }
    cout << "Your Tasks : " << end1;
    for(int i = 0; i < tasks.size(); i++) {
        cout << i + 1 << "." << task[i] << end1;
    }
}
//   3. Delete a Task
